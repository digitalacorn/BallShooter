...
timer = 0.0
while STARTSCREEN and not EXITGAME:
...
timer = 0
while ENDSCREEN and not EXITGAME:
	pygame.draw.circle(screen, CURSORCOLOUR, mouse_pos, CURSORRADIUS, 2)
	screen.fill(BKGCOLOUR)
	timer += 1
	for event in pygame.event.get():
...
SCORESCREEN = True
while SCORESCREEN and not EXITGAME:
	pygame.draw.circle(screen, CURSORCOLOUR, mouse_pos, CURSORRADIUS, 2)
	screen.fill(BKGCOLOUR)
	pos = 1
	surfi = []
	...
	so.reverse()
	for score in so[:10]:
		score_surface = myFont.render("rank:%i score:%i"%(pos, score), 3, LINECOLOUR)
